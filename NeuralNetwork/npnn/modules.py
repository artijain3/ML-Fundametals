...
class ELU(Module):
    ...
    def forward(self, x, train=True):
        ...
        """
        self.x = x # assuming that x is batchxdim
            
        elu_output = np.zeros((self.x.shape))
        for row in range(self.x.shape[0]): # iterating through the rows
            elu_output[row] = np.where(x[row] > 0, x[row], self.alpha*(np.exp(x[row])-1))
        return elu_output
    ...
    def backward(self, grad):
        ...
        """
        x = self.x
        dLdx = np.zeros((grad.shape)) # batch x dim
        for row in range(grad.shape[0]): # go through each row in the grad -> 1xdim
            temp = np.diag(np.where(self.x[row] > 0, 1, self.alpha * (np.exp(self.x[row])))) # 1 x dim
            dLdx[row] = grad[row] @ temp # TODO: maybe dLdx * grad, 1xdim * 1xdim
...
class Dense(Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        # TODO
        W = np.random.uniform(low=-np.sqrt(6/(dim_in+dim_out)), high=np.sqrt(6/(dim_in+dim_out)), size=(dim_out,dim_in)) # size should dim_out x dim_in
        b = np.zeros(dim_out) # should maybe be dim_out?
    ...
    def forward(self, x, train=True):
        ...
        np.array
            ...
            (batch, dim).
        """
        self.x = x # batch x dim_in
        W, b = self.trainable_weights # W: dim_in x dim_out, b: dim_out x 1
        
        return (self.x @ W.value.T) + b.value # batch x dim_out
    ...
    def backward(self, grad):
        ...
        # TODO
        # W.grad = dL/dW_k = ?
        W.grad = grad.T @ self.x # dim_in x dim_out
        W.grad /= batch
        # b.grad = dL/db_k = ?
        b.grad = grad.sum(axis=0) # because dx/db = identity -> batch x dim_out -> dim_out x 1
        b.grad /= batch
        # dx = dL/dx_{k-1} = ?
        dx = grad @ W.value
